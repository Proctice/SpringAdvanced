...
3. ApplicationContext 比 BeanFactory 多点啥
   ...
   * 国际化、通配符方式获取一组 Resource 资源、整合 Environment 环境、事件发布与监听(EnvironmentCapable,  MessageSource, ApplicationEventPublisher, ResourcePatternResolver )
   * 新学一种代码之间解耦途径，事件解耦
...
>
* 版本选择了 java 8, 因为目前的 aspectj-maven-plugin 1.14.0 最高只支持到 java 16
* 运行时, 设置vm参数 -javaagent:C:/Users/manyh/.m2/repository/org/aspectj/aspectjweaver/1.9.7/aspectjweaver-1.9.7.jar
* 查看运行时的字节码(本地编译没有经过agent增强): arthas 
```
java -jar arthas.jar
jad 全路径    
```
...
public class CglibProxyDemo {
	...
	public static void main(String[] param) {
		...
		Target proxy = (Target) Enhancer.create(Target.class, (MethodInterceptor) (p, method, args, methodProxy) -> {
			...
			// Object result = method.invoke(target, args); // 方式一 用方法反射调用目标
			// methodProxy 它可以避免反射调用
			// Object result = methodProxy.invoke(target, args); //方式二 内部没有用反射, 需要目标 （spring）
			Object result = methodProxy.invokeSuper(p, args); // 方式三 内部没有用反射, 需要代理
...
* 注意点 *
> 使用asm技术生成字节码(spring, jdk都有实现)
> idea安装asm插件, 在asm模块测试, 插件仅适用于jdk8
...
**com.itheima.a14.ProxyFastClass**，**com.itheima.a14.TargetFastClass**
